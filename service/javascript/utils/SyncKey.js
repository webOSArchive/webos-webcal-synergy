...
SyncKey.prototype.prepare = function (kindName, state) {
	...
	if (state === "first") {
		...
		// large calendars. The per-batch saveErrorState already checkpoints ctags, so resuming
		// from the last ctag is safe — the server will resend any events that changed after it.
